...
{
    public class LogPController : Controller
    {
        private readonly LabContext _context;
        private readonly IHttpContextAnalysisService _analysis;
        private readonly ILoggerService _logger;
        public LogPController(LabContext context,  IHttpContextAnalysisService analysis, ILoggerService logger)
        {
            _context = context;
            _analysis = analysis;
            _logger = logger;
        ...
        public IActionResult Page([Required] Int32 index, [Required] String pId, [Required] int code,[Required] DateTime left,[Required] DateTime right)
        {
            if (ModelState.IsValid)
            {
                StringBuilder builder = new StringBuilder("select * from  LogPricipalOperations where LogPricipalOperationId > 0");
                List<SqlParameter> parameters = new List<SqlParameter>();

                if (pId != null && pId.Trim() != "" && pId.Trim() != "-1")
                {
                    builder.Append(" and PrincipalId = @Id");
                ...
                if (code >= 0)
                {
                    builder.Append($" and PrincpalOperationCode = {code}");
                }
                int pageSize = 12; //每一页的题目数量
                // ReSharper disable once CoVariantArrayConversion
                int dataCount = _context.LogPricipalOperations.FromSql(builder.ToString(), parameters.ToArray<SqlParameter>()).Count();
                ...
                // ReSharper disable once CoVariantArrayConversion
                var list = _context.LogPricipalOperations
